...
'''
Original file from https://github.com/pucrs-automated-planning/pddl-parser/blob/master/PDDL.py
Commit id: 7cbfb40b8918484706241498882413826767bc35

Reusing original file for parsing, no copyrights intended.
'''
...
class PDDL_Parser:
    ...
    SUPPORTED_REQUIREMENTS = [':strips', ':negative-preconditions', ':equality', ':typing', 'constants']
            ...
            while tokens:
                ...
                if   t == 'problem':
                    ...
                    while group:
                        if group[0] == '-':
                            group.pop(0)
                            obj_type = group.pop(0)
                            if obj_type in self.objects:
                                self.objects[obj_type].extend(object_list)
                            else:
                                self.objects[obj_type] = object_list
                            object_list = []
                        else:
